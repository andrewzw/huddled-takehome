...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  const engagementLineQuery = `
    WITH metrics AS (
      ...
      FROM user_events ue
      JOIN artists a ON ue.artist_id = a.id --join artist with ue table using id from artist table
      WHERE ue.event_type IN ('like_track', 'add_track_to_playlist', 'play_track', 'share_track')
    )
    SELECT 
      artist_name,
      CASE period_type    --asign values depending on type
        WHEN 'hourly' THEN hour 
        ELSE NULL -- null irrelevant
      END as hour,
      CASE period_type 
      ...
      period_type
    FROM metrics
    CROSS JOIN ( --join metrics with periods table to get values for each period
  ...
  const engagementStats = await db.prepare(engagementLineQuery).all();
  const eventBarStatsAvg = await db.prepare(eventBarStatsQuery).all();
  ...
  const eventStackBarStats = await db.prepare(eventStackBarStatsQuery).all();
