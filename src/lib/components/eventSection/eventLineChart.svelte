<script lang="ts">
  ...
  function processTimeData(
    ...
    if (data && !isNaN(timeValue)) {
      data[timeValue + adjustment] += points
  ...
  function processData() {
    ...
    const config =
      periodConfig[viewType as "hourly" | "daily" | "monthly" | "yearly"];
            ...
            text:
              ...
              (viewType === "hourly"
                ...
                : viewType === "daily"
                  ...
                  : `Days of Month Engagement by `) +
              `(${points ? "Points" : "Occurrences"})`,
            color: "#f5fefd",
            font: { size: 16 },
          },
          legend: {
